...
void fn_cd (inode_state& state, const wordvec& words) {
   ...
   if(words.size() > 2){
      throw command_error ("Usage: cd [todir]");
      exec::status(1);
      return;
   }
   ...
   if(target_inode == nullptr ||
    target_inode->type() == file_type::PLAIN_TYPE){
      throw command_error ("can not cd " + target_pathname);
      exec::status(1);
      return;
   }
...
void fn_lsr (inode_state& state, const wordvec& words) {
   if (words.size() > 2){
      throw command_error ("Usage: lsr [pathname]");
      exec::status(1);
      return;
   }
   ...
   inode_ptr target_inode = get_inode(state.get_root(), pathname);
   if(target_inode == nullptr){
      throw command_error ("path" + pathname + " does not exis");
      exec::status(1);
      return;
   }
   if(target_inode->type() == file_type::PLAIN_TYPE){
      throw command_error ("can not run lsr on a plain file");
      exec::status(1);
      return;
   }
...
void fn_make (inode_state& state, const wordvec& words) {
   ...
   if(words.size() <= 1){
      throw command_error ("Usage: make <pathname> [data]");
      exec::status(1);
      return;
   }
   ...
   if(parent_inode == nullptr){
      throw command_error (parent_pathname + " doesn't exist");
      exec::status(1);
      return;
   }
   inode_ptr target_inode = 
   ...
   file_type::DIRECTORY_TYPE){
      throw command_error (target_pathname + " is a directory");
      exec::status(1);
      return;
   }
...
void fn_mkdir (inode_state& state, const wordvec& words) {
   if(words.size() != 2){
      throw command_error ("Usage: mkdir <dir_name>");
      exec::status(1);
      return;
   }
   ...
   if(target_inode != nullptr){
      throw command_error (target_pathname + " already exist");
      exec::status(1);
      return;
   }
...
void fn_prompt (inode_state& state, const wordvec& words) {
   ...
   DEBUGF ('c', words);
   if(words.size() != 2){
      throw command_error ("Usaeg: prompt <prompt>");
      exec::status(1);
      return;
   }
...
void fn_rm (inode_state& state, const wordvec& words) {
   if(words.size() != 2){
      throw command_error ("Usage: rm <pathname>");
      exec::status(1);
      return;
   }
   ...
   inode_ptr parent_inode=get_inode(state.get_root(), parent_pathname);
   if(parent_inode == nullptr){
      throw command_error (parent_pathname+ " does not exist");
      exec::status(1);
      return;
   }
   inode_ptr target_inode=get_inode(state.get_root(), target_pathname);
   if(target_inode == nullptr){
      throw command_error (target_pathname + " does not exist");
      exec::status(1);
      return;
   }
...
void fn_rmr (inode_state& state, const wordvec& words) {
   if (words.size() > 2){
      throw command_error ("Usage: rmr [pathname]");
      exec::status(1);
      return;
   }
   if(words[1] == "/"){
   ...
   inode_ptr target_inode = get_inode(state.get_root(), pathname);
   if(target_inode == nullptr){
      throw command_error ("path " + pathname+" does not exist");
      exec::status(1);
      return;
   }
   if(target_inode->type() == file_type::PLAIN_TYPE){
      throw command_error ("can not run rmr on a plain file");
      exec::status(1);
      return;
   }
