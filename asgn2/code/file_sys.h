...
class inode_state {
   ...
   public:
      ...
      void set_cwd(inode_ptr _cwd) { cwd = _cwd; }
      void set_root(inode_ptr _root) { root = _root; }
      void set_prompt(string prompt) { prompt_ = prompt; }
};
...
class inode {
   ...
   public:
      inode() = delete;    
      //Ref: https://stackoverflow.com/questions/5513881/meaning-of-delete-after-function-declaration
      ...
      const std::string& get_contents_type();
      void write (const wordvec& newdata);
      file_type type();
      size_t size();
};
...
class base_file {
   ...
   public:
      ...
      virtual size_t size() const = 0;    // Pure vitural function.
